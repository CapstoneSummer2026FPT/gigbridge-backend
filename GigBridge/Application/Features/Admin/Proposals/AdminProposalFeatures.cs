...
public sealed record AdminProposalListItem(Guid ProposalId, Guid JobPostId, string JobPostTitle,
    Guid ClientId, string ClientName, Guid FreelancerId, string FreelancerName, decimal? ProposedBudget,
...
public sealed record AdminProposalParty(Guid UserId, string Name, string? Summary, int AccountStatus,
...
public sealed record AdminProposalNegotiationOffer(Guid OfferId, Guid ConversationId, Guid CreatedByUserId,
    string CreatedByName, decimal Budget, DateOnly? StartDate, DateOnly? EndDate, string? Scope,
...
public sealed record AdminProposalNote(Guid NoteId, Guid AdminId, string AdminName, string Content, DateTime CreatedAt);
public sealed record AdminProposalAudit(Guid AuditId, Guid AdminId, string AdminName, string Action,
    ...
    IRequestHandler<GetAdminProposalAggregateQuery, AdminProposalDetail>
{
    ...
    public async Task<PaginatedList<AdminProposalListItem>> Handle(AdminProposalListQuery q, CancellationToken ct)
    {
        ...
        var items = await query.Skip((page - 1) * size).Take(size).Select(x => new AdminProposalListItem(x.ProposalsId, x.JobPostsId, x.JobPosts.Title, x.JobPosts.ClientProfiles.UserId, x.JobPosts.ClientProfiles.User.FullName, x.FreelancerProfiles.UserId, x.FreelancerProfiles.User.FullName, x.ProposedBudget, x.ProposedDuration, x.SubmittedAt, x.UpdatedAt, x.Status, x.ModerationStatus,
    ...
    public async Task<AdminProposalDetail> Handle(GetAdminProposalAggregateQuery q, CancellationToken ct)
    {
        ...
        var offerDtos = offers.Select(o => new AdminProposalNegotiationOffer(o.NegotiationOfferId, o.ConversationsId, o.Status == 0 ? o.ClientProfiles.UserId : o.FreelancerProfiles.UserId, o.Status == 0 ? o.ClientProfiles.User.FullName : o.FreelancerProfiles.User.FullName, o.FinalPrice, o.StartDate, o.EndDate, o.ScopeSummary, o.Status, o.CreatedAt, o.RespondedAt, o.NegotiationOfferMilestones.OrderBy(m => m.OrderIndex).Select(m => new AdminProposalNegotiationMilestone(m.Title, m.Description, m.Amount, m.EstimatedDuration, m.DueDate, m.WorkItems.OrderBy(w => w.OrderIndex).Select(w => new AdminProposalWorkItem(w.NegotiationOfferWorkItemId, w.Title, w.Description, w.Deliverables, w.EstimatedDuration, w.OrderIndex)).ToList())).ToList())).ToList();

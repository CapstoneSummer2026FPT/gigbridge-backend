    ...
    IRequestHandler<GetAdminContractReportAuditQuery, IReadOnlyList<AdminContractReportAudit>>
{
    ...
    public AdminContractReportQueryHandler(IApplicationDbContext context, IAdminAuditService audit)
    {
        _context = context;
        _audit = audit;
    }
    ...
    public async Task<PaginatedList<AdminContractReportListItem>> Handle(GetAdminContractReportsQuery q, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(q.Search))
        {
            ...
            query = query.Where(x => x.Description.ToLower().Contains(s) || x.Contract.Title.ToLower().Contains(s) || x.Reporter.FullName.ToLower().Contains(s) || (x.Respondent != null && x.Respondent.FullName.ToLower().Contains(s)));
        }
        ...
        if (q.UnassignedOnly)
            query = query.Where(x => x.AssignedAdminId == null);
        else if (q.AssignedAdminId.HasValue)
            query = query.Where(x => x.AssignedAdminId == q.AssignedAdminId);
    ...
    public async Task<AdminContractReportDetail> Handle(GetAdminContractReportDetailQuery q, CancellationToken ct)
    {
        ...
        var client = Party(c.ClientProfiles.User, "Client");
        var freelancer = c.FreelancerProfiles is null ? null : Party(c.FreelancerProfiles.User, "Freelancer");
        ...
        var final = IsFinal(r.AdminReviewStatus);
        var linkedOpen = related is not null && related.Status is not (int)DisputeStatus.Resolved and not (int)DisputeStatus.Closed;
    ...
    IRequestHandler<DismissContractReportCommand, AdminContractReportDetail>, IRequestHandler<LinkContractReportDisputeCommand, AdminContractReportDetail>
{
    ...
    private readonly ILogger<AdminContractReportMutationHandler> _logger;

    public AdminContractReportMutationHandler(IApplicationDbContext context, IAdminAuditService audit, IMediator mediator, IDateTimeService clock, INotificationService notifications, ILogger<AdminContractReportMutationHandler> logger)
    {
        _context = context;
        _audit = audit;
        _mediator = mediator;
    ...
    {
        await using var tx = await Begin(q.ActorAdminId, q.ReportId, ct);
        var r = await Load(q.ReportId, ct);
        EnsureOpen(r);
        var target = q.AdminId ?? q.ActorAdminId;
        var admin = await _context.Set<User>().FirstOrDefaultAsync(x => x.UserId == target && x.Role == (int)UserRole.Admin && x.IsActive && x.AccountStatus == (int)AccountStatus.Active, ct) ?? throw new BadRequestException("The assigned user must be an active administrator.");
        var old = new { r.AssignedAdminId, r.AssignedAt, r.AdminReviewStatus };
        var action = r.AssignedAdminId.HasValue ? AdminAuditActions.ContractReportReassigned : AdminAuditActions.ContractReportAssigned;
        r.AssignedAdminId = admin.UserId;
        r.AssignedAt = _clock.UtcNow;
        r.AdminReviewStatus = (int)ContractReportAdminStatus.UnderReview;
        r.UpdatedAt = _clock.UtcNow;
        _audit.Add(q.ActorAdminId, action, nameof(ReportContract), r.ReportContractId, old, new { r.AssignedAdminId, r.AssignedAt, r.AdminReviewStatus, r.ContractId, r.ReporterId, r.RespondentId });
        await SaveCommit(tx, ct);
    ...
    public async Task<AdminContractReportDetail> Handle(AddContractReportNoteCommand q, CancellationToken ct)
    {
        ...
        await using var tx = await Begin(q.AdminId, q.ReportId, ct);
        var r = await Load(q.ReportId, ct);
        ...
        _context.Set<ReportContractAdminNote>().Add(note);
        r.UpdatedAt = _clock.UtcNow;
        _audit.Add(q.AdminId, AdminAuditActions.ContractReportInternalNoteAdded, nameof(ReportContract), r.ReportContractId, null, new { noteId = note.ReportContractAdminNoteId, r.ContractId });
        await SaveCommit(tx, ct);
        return await Detail(r.ReportContractId, ct);
    }
    ...
    {
        if (q.Request.RequestId == Guid.Empty) throw new BadRequestException("Request ID is required.");
        if (string.IsNullOrWhiteSpace(q.Request.Message)) throw new BadRequestException("A request message is required.");
        if (q.Request.DueAt.HasValue && q.Request.DueAt <= _clock.UtcNow) throw new BadRequestException("Due date must be in the future.");
        await using var tx = await Begin(q.AdminId, q.ReportId, ct);
        var r = await Load(q.ReportId, ct);
        EnsureOpen(r);
        ...
        _context.Set<ReportContractInformationRequest>().AddRange(rows);
        r.AssignedAdminId ??= q.AdminId;
        r.AssignedAt ??= _clock.UtcNow;
        r.AdminReviewStatus = (int)ContractReportAdminStatus.AwaitingInformation;
        r.UpdatedAt = _clock.UtcNow;
        _audit.Add(q.AdminId, AdminAuditActions.ContractReportInformationRequested, nameof(ReportContract), r.ReportContractId, null, new { q.Request.RequestId, targets, q.Request.DueAt, r.ContractId, r.ReporterId, r.RespondentId });
        await SaveCommit(tx, ct);
        foreach (var target in targets)
        {
            try
            {
                await _notifications.CreateNotificationAsync(target, NotificationType.ReportUpdate, "Additional contract report information requested", q.Request.Message.Trim(), r.ReportContractId, nameof(ReportContract), ct);
            }
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
                _logger.LogWarning(ex, "Failed to notify user {UserId} about contract report {ReportContractId} information request.", target, r.ReportContractId);
            }
        }
        return await Detail(r.ReportContractId, ct);
    }
    ...
    public async Task<AdminContractReportDetail> Handle(CloseContractReportCommand q, CancellationToken ct)
    {
        ...
        var r = await Load(q.ReportId, ct);
        EnsureOpen(r);
        var linkedOpen = await _context.Set<Dispute>().AnyAsync(x => x.RelatedReportId == r.ReportContractId && x.Status != (int)DisputeStatus.Resolved && x.Status != (int)DisputeStatus.Closed, ct);
        if (linkedOpen) throw new ConflictException("A Contract Report cannot be closed while its related Dispute remains open.");
        var old = Snapshot(r);
        r.AdminReviewStatus = (int)ContractReportAdminStatus.Closed;
        r.AdminResolutionAction = (int)q.Request.ResolutionAction;
        r.AdminResolutionNote = q.Request.ResolutionSummary.Trim();
        r.Status = (int)ContractReportStatus.Resolved;
        r.ResolvedBy = q.AdminId;
        r.ResolvedAt = _clock.UtcNow;
        r.UpdatedAt = _clock.UtcNow;
        AddOptionalNote(r, q.AdminId, q.Request.InternalNote);
        _audit.Add(q.AdminId, AdminAuditActions.ContractReportClosed, nameof(ReportContract), r.ReportContractId, old, Snapshot(r));
        await SaveCommit(tx, ct);
        return await Detail(r.ReportContractId, ct);
    }
    ...
    public async Task<AdminContractReportDetail> Handle(DismissContractReportCommand q, CancellationToken ct)
    {
        ...
        EnsureOpen(r);
        var old = Snapshot(r);
        r.AdminReviewStatus = (int)ContractReportAdminStatus.Dismissed;
        r.AdminResolutionNote = q.Request.Reason.Trim();
        r.Status = (int)ContractReportStatus.Resolved;
        r.ResolvedBy = q.AdminId;
        r.ResolvedAt = _clock.UtcNow;
        r.UpdatedAt = _clock.UtcNow;
        AddOptionalNote(r, q.AdminId, q.Request.InternalNote);
        _audit.Add(q.AdminId, AdminAuditActions.ContractReportDismissed, nameof(ReportContract), r.ReportContractId, old, Snapshot(r));
        await SaveCommit(tx, ct);
        return await Detail(r.ReportContractId, ct);
    }
    ...
    public async Task<AdminContractReportDetail> Handle(LinkContractReportDisputeCommand q, CancellationToken ct)
    {
        ...
        var r = await Load(q.ReportId, ct);
        EnsureOpen(r);
        await tx.AcquireTransactionLockAsync(ContractEscrowLock.ForContract(r.ContractId), ct);
        ...
        var participantIds = new[] { dispute.Contracts.ClientProfiles.UserId, dispute.Contracts.FreelancerProfiles?.UserId }.Where(x => x.HasValue).Select(x => x!.Value).ToHashSet();
        if (!participantIds.Contains(r.ReporterId) || (r.RespondentId.HasValue && !participantIds.Contains(r.RespondentId.Value))) throw new BadRequestException("The Report and Dispute participants do not match.");
        var old = Snapshot(r);
        dispute.RelatedReportId = r.ReportContractId;
        r.Status = (int)ContractReportStatus.Escalated;
        r.IsEscalatedToDispute = true;
        r.AdminReviewStatus = (int)ContractReportAdminStatus.LinkedToDispute;
        r.AdminResolutionNote = q.Request.Reason.Trim();
        r.UpdatedAt = _clock.UtcNow;
        _audit.Add(q.AdminId, AdminAuditActions.ContractReportLinkedToDispute, nameof(ReportContract), r.ReportContractId, old, new { report = Snapshot(r), disputeId = dispute.DisputesId, r.ContractId, r.ReporterId, r.RespondentId });
        await SaveCommit(tx, ct);
        return await Detail(r.ReportContractId, ct);
    }

    private async Task<IApplicationDbContextTransaction> Begin(Guid adminId, Guid reportId, CancellationToken ct)
    {
        await EnsureAdmin(adminId, ct);
        var tx = await _context.BeginTransactionAsync(ct);
        await tx.AcquireTransactionLockAsync(ReportContractLock.ForReport(reportId), ct);
        return tx;
    }

    private async Task EnsureAdmin(Guid id, CancellationToken ct)
    {
        if (!await _context.Set<User>().AsNoTracking().AnyAsync(x => x.UserId == id && x.Role == (int)UserRole.Admin && x.IsActive && x.AccountStatus == (int)AccountStatus.Active, ct)) throw new ForbiddenAccessException("An active administrator account is required.");
    }
    ...
    private static void EnsureOpen(ReportContract r)
    {
        if (AdminContractReportQueryHandler.IsFinal(r.AdminReviewStatus)) throw new ConflictException("This Contract Report is already finalized.");
    }

    private void AddOptionalNote(ReportContract r, Guid adminId, string? content)
    {
        if (!string.IsNullOrWhiteSpace(content)) _context.Set<ReportContractAdminNote>().Add(new() { ReportContractAdminNoteId = Guid.NewGuid(), ReportContractId = r.ReportContractId, AdminUserId = adminId, Content = content.Trim(), CreatedAt = _clock.UtcNow });
    }
    ...
    private async Task SaveCommit(IApplicationDbContextTransaction tx, CancellationToken ct)
    {
        try
        {
            await _context.SaveChangesAsync(ct);
        ...
        {
            throw new ConflictException("The Contract Report changed while it was being processed. Refresh and retry.");
        }
        catch (DbUpdateException ex) when (ex.InnerException?.Message.Contains("InformationRequests", StringComparison.OrdinalIgnoreCase) == true || ex.InnerException?.Message.Contains("RelatedReportId", StringComparison.OrdinalIgnoreCase) == true)
        {
            throw new ConflictException("This action has already been processed or conflicts with another request.");
        }
    }
...
public sealed class GetContractReportAttachmentDownloadHandler : IRequestHandler<GetContractReportAttachmentDownloadQuery, ContractReportAttachmentDownload>
{
    ...
    public GetContractReportAttachmentDownloadHandler(IApplicationDbContext context, IAdminAuditService audit)
    {
        _context = context;
        _audit = audit;
    }

    public async Task<ContractReportAttachmentDownload> Handle(GetContractReportAttachmentDownloadQuery q, CancellationToken ct)
    {
        if (!await _context.Set<User>().AnyAsync(x => x.UserId == q.AdminId && x.Role == (int)UserRole.Admin && x.IsActive, ct)) throw new ForbiddenAccessException("Admin access is required.");
        var a = await _context.Set<ReportContractAttachment>().AsNoTracking().FirstOrDefaultAsync(x => x.ReportContractId == q.ReportId && x.ReportContractAttachmentId == q.AttachmentId, ct) ?? throw new NotFoundException("Contract Report attachment does not exist.");
        _audit.Add(q.AdminId, AdminAuditActions.ContractReportEvidenceDownloaded, nameof(ReportContract), q.ReportId, null, new { attachmentId = a.ReportContractAttachmentId, a.FileName });
        await _context.SaveChangesAsync(ct);
        return new(a.ReportContractAttachmentId, a.FileName, a.FileUrl);
    }
}

...
public sealed class AccountReportQueryHandler : IRequestHandler<GetAccountReportsQuery, PaginatedList<AccountReportListItemDto>>, IRequestHandler<GetAccountReportDetailQuery, AccountReportDetailDto>
{
    ...
    public AccountReportQueryHandler(IApplicationDbContext context) => _context = context;

    private sealed class AccountReportRow
    {
    ...
    public async Task<PaginatedList<AccountReportListItemDto>> Handle(GetAccountReportsQuery q, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(q.Search))
        {
            ...
            query = query.Where(x => x.Report.Reason.ToLower().Contains(term) || x.Report.Reporter.FullName.ToLower().Contains(term) || x.Target.FullName.ToLower().Contains(term) || x.Target.Email.ToLower().Contains(term));
        }
...
public sealed class AccountReportMutationHandler : IRequestHandler<UpdateAccountReportStatusCommand, AccountReportDetailDto>, IRequestHandler<ResolveAccountReportCommand, AccountReportDetailDto>
{
    ...
    public AccountReportMutationHandler(IApplicationDbContext context, IUserAccountStatusService status, IAdminAuditService audit, IMediator mediator)
    {
        _context = context;
        _status = status;
        _audit = audit;
    ...
    public async Task<AccountReportDetailDto> Handle(UpdateAccountReportStatusCommand q, CancellationToken ct)
    {
        ...
        await tx.AcquireTransactionLockAsync(AccountEnforcementLock.ForReport(q.ReportId), ct);
        var report = await Load(q.AdminId, q.ReportId, ct);
        EnsureOpen(report);
        var old = new { report.Status, report.AssignedAdminId, report.AdminNote };
        report.Status = (int)q.Request.Status;
        report.AssignedAdminId ??= q.AdminId;
        report.AssignedAt ??= DateTime.UtcNow;
        report.AdminNote = q.Request.AdminNote?.Trim();
        report.UpdatedAt = DateTime.UtcNow;
        if (q.Request.Status == ReportStatus.Dismissed)
        {
            report.ResolvedByAdminId = q.AdminId;
            report.ResolvedAt = DateTime.UtcNow;
            report.ResolutionAction = (int)AccountReportResolutionAction.None;
        }
        ...
        await SaveResolutionAsync(ct);
        await tx.CommitAsync(ct);
        return await _mediator.Send(new GetAccountReportDetailQuery(report.ReportsId), ct);
    }
    ...
    public async Task<AccountReportDetailDto> Handle(ResolveAccountReportCommand q, CancellationToken ct)
    {
        ...
        var report = await Load(q.AdminId, q.ReportId, ct);
        EnsureOpen(report);
        await tx.AcquireTransactionLockAsync(AccountEnforcementLock.ForUser(report.ReportedEntityId), ct);
        var user = await _context.Set<User>().FirstAsync(x => x.UserId == report.ReportedEntityId, ct);
        if (user.Role == (int)UserRole.Admin) throw new ConflictException("Admin accounts are protected from account-report enforcement.");
        ...
        report.ResolutionAction = (int)q.Request.Action;
        report.AdminNote = q.Request.AdminNote?.Trim();
        report.ResolvedByAdminId = q.AdminId;
        report.ResolvedAt = DateTime.UtcNow;
        report.AssignedAdminId ??= q.AdminId;
    ...
    private async Task<Report> Load(Guid adminId, Guid id, CancellationToken ct)
    {
        ...
        return await _context.Set<Report>().FirstOrDefaultAsync(x => x.ReportsId == id && x.ReportedEntityType == ReportedEntityTypes.User, ct) ?? throw new NotFoundException("Account report does not exist.");
    }

    private static void EnsureOpen(Report r)
    {
        if (r.Status is (int)ReportStatus.Resolved or (int)ReportStatus.Dismissed) throw new ConflictException("This report has already been finalized.");
    }
    ...
    private async Task SaveResolutionAsync(CancellationToken ct)
    {
        ...
        {
            throw new ConflictException("The report changed while it was being processed. Refresh and retry.");
        }
